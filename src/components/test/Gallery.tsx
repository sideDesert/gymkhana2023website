...
function Gallery({imageArray=images}:{imageArray:string[]}) {
  ...
  return (
          <div className='flex flex-row p-8 justify-center items-center'>
            <button className='p-4 bg-slate-400 h-fit mr-3' onClick={prevSlide}>Prev</button>
            <div ref={carouselRef} className='carousel h-[300px] w-[500px] bg-blue-500 flex flex-row overflow-hidden'>
              {imageArray.map((image,index)=>(
                <div className={`image${index} h-[300px] w-[500px] object-cover flex-shrink-0`} key={image}>
                  <img src={image} alt="" className='object-cover w-full h-full'/>
                </div>
              ))}
            </div>
            <button className='p-4 bg-slate-400 h-fit ml-3' onClick={nextSlide}>Next</button>
        </div>
  )
}
